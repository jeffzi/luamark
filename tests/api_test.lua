...
describe("suite API (compare_time/compare_memory)", function()
   ...
   describe("funcs validation", function()
      test("compare_time rejects array (numeric keys)", function()
         assert.has_error(function()
            luamark.compare_time({ h.noop, h.noop }, { rounds = 1 })
         end, "'funcs' keys must be strings, got number. Use named keys: { name = fn }")
      end)

      test("compare_time rejects mixed keys", function()
         assert.has_error(function()
            luamark.compare_time({ h.noop, named = h.noop }, { rounds = 1 })
         end, "'funcs' keys must be strings, got number. Use named keys: { name = fn }")
      end)

      test("compare_memory rejects array (numeric keys)", function()
         assert.has_error(function()
            luamark.compare_memory({ h.noop, h.noop }, { rounds = 1 })
         end, "'funcs' keys must be strings, got number. Use named keys: { name = fn }")
      end)

      test("compare_memory rejects mixed keys", function()
         assert.has_error(function()
            luamark.compare_memory({ h.noop, named = h.noop }, { rounds = 1 })
         end, "'funcs' keys must be strings, got number. Use named keys: { name = fn }")
      end)
   end)
end)
