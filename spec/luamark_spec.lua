...
describe("suite", function()
   ...
   describe("set_nested validation", function()
      test("rejects table param values", function()
         assert.has.error(function()
            luamark._internal.set_nested({}, { n = {} }, "stats")
         end, "param 'n' has invalid type 'table' (must be string, number, or boolean)")
      end)

      test("rejects function param values", function()
         assert.has.error(function()
            luamark._internal.set_nested({}, { f = function() end }, "stats")
         end, "param 'f' has invalid type 'function' (must be string, number, or boolean)")
      end)

      test("rejects NaN param values", function()
         assert.has.error(function()
            luamark._internal.set_nested({}, { n = 0 / 0 }, "stats")
         end, "param 'n' cannot be NaN or infinite")
      end)

      test("rejects infinite param values", function()
         assert.has.error(function()
            luamark._internal.set_nested({}, { n = math.huge }, "stats")
         end, "param 'n' cannot be NaN or infinite")
      end)

      test("rejects negative infinite param values", function()
         assert.has.error(function()
            luamark._internal.set_nested({}, { n = -math.huge }, "stats")
         end, "param 'n' cannot be NaN or infinite")
      end)
